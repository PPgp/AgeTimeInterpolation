...
for(LocID in country.codes) {
	...
	strt<-Sys.time()
	## mx5 <- data.table(read_csv(paste0(mx5x5.prefix, LocID, '.csv'), col_names = TRUE, skip = 0, n_max = -1, progress = interactive()))
	mx5 <- fread(mx5.file.name, header = TRUE, skip = 0, nrows=-1, showProgress=TRUE, data.table=TRUE)
...
##  Tried to fit ICM Mortpak model but too slow - decided to use analytical form (Keyfitz rationale function)
##	
...
##	rm("mx10.long")
##	
##	## fwrite(u10lt, paste0("ICM_", LocID, ".csv"), append = FALSE, col.names = TRUE)
##	## fwrite(u10lt[Age==0], paste0("ICM0.csv"), append = TRUE, col.names = TRUE)
##	## fwrite(u10lt[Age==0 & value <= 0.00000001], paste0("ICM0-good.csv"), append = TRUE, col.names = TRUE)
##	## fwrite(u10lt[(Age==0 & value <= 0.00000001), .(p1.min =min(p1), p1.mean=mean(p1), p1.max=max(p1), p2.min =min(p2), p2.mean=mean(p2), p2.max=max(p2), p3.min =min(p3), p3.mean=mean(p3), p3.max=max(p3))], paste0("ICM0-good-stats.csv"), append = TRUE, col.names = TRUE)
##
##	## fwrite(u10lt[GoF > 0.00000001], paste0("ICM-bad.csv"), append = TRUE, col.names = TRUE)
##	## fwrite(u10lt[(Age==0 & value > 0.00000001), .(p1.min =min(p1), p1.mean=mean(p1), p1.max=max(p1), p2.min =min(p2), p2.mean=mean(p2), p2.max=max(p2), p3.min =min(p3), p3.mean=mean(p3), p3.max=max(p3))], paste0("ICM0-bad-stats.csv"), append = TRUE, col.names = TRUE)
##	
#### }	
#### LocID	SexID	MidPeriod	Trajectory	Age	mx	qx	q5	q5.fit	p1	p2	p3	value
##	
##	
##	# print(paste0("param1: ", range(param1, na.rm=TRUE)))
##	# print(paste0("param2: ", range(param2, na.rm=TRUE)))
##	# print(paste0("param3: ", range(param3, na.rm=TRUE)))
##	## u10lt.wide <- dcast(u10lt, LocID + SexID + Trajectory + Age ~ MidPeriod, value.var="mx")
##	## round(head(u10lt.wide, 30), 6)
